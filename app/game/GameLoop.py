                ...
                while not end_game and not close_game and not end_turn:
                    if not player.IA:
                        ...
                        # Clicar em finalizar jogada
                            if event.type == pygame.MOUSEBUTTONDOWN and jogo.buttons.buttnMeldRect.collidepoint(
                                ...
                                if not player.doneMeld:
                                    player.doneMeld = True if player.initial_meld(player_pieces_placed,
                                                                                  jogo.table.get_groups()) >= 30 else False
                                ...
                                if not is_moving_piece:
                                    ...
                                    for line_tab in range(len(jogo.table.tabuleiro)):
                                        ...
                                        for col_tab in range(len(jogo.table.tabuleiro[line_tab])):
                                            ...
                                            if tab.value != PieceValue.BLANK and tab.rect.collidepoint(
                                                    pygame.mouse.get_pos()):
                                                is_moving_piece = True
                                    ...
                                    if is_moving_piece:
                                        ...
                                        if x == -1:
                                            ...
                                            if player.hand[tile_moving].whereAt == PieceLocale.HAND:
                                                player.hand[tile_moving].rect.x, player.hand[tile_moving].rect.y = \
                                                player.hand[tile_moving].originalPlace
                                            else:
                        ...
                        if is_moving_piece and (type(tile_moving) is int or type(tile_moving) is str):
                            ...
                            jogo.table.tabuleiro[tile_moving[0]][tile_moving[1]].rect.x = pygame.mouse.get_pos()[0] - \
                                                                                          mouse_offset[0]
                            jogo.table.tabuleiro[tile_moving[0]][tile_moving[1]].rect.y = pygame.mouse.get_pos()[1] - \
                                                                                          mouse_offset[1]
